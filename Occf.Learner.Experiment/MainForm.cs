...
using Occf.Learner.Core.Tests;
using Occf.Learner.Core.Tests.Experiments;
using Paraiba.Linq;
using Sgry.Azuki;
...
namespace Occf.Learner.Experiment {
	public partial class MainForm : Form {
		...
		private Processor _activeProcessor;
		private CodeFile _activeFile;
		private ListViewItem _lastSelectedItem;
		private bool _eventEnabled;
		private BitLearningExperimentGroupingWithId _activeExperiment;
		private int _count;
		...
		private void MainForm_Load(object sender, EventArgs e) {
			...
			var processors = new Processor[] {
				ProcessorLoader.JavaUsingAntlr3,
				ProcessorLoader.CSharpUsingAntlr3,
				ProcessorLoader.JavaScriptUsingAntlr3,
				ProcessorLoader.PhpUsingAntlr3,
				ProcessorLoader.LuaUsingAntlr3,
			};
			foreach (var processor in processors) {
				var menuItem =
						menuItemLanguage.DropDownItems.Add(processor.FullLanguageName) as ToolStripMenuItem;
					...
					path =>
							Directory.Exists(path)
									? Directory.EnumerateFiles(path, "*", SearchOption.AllDirectories)
											.Where(
													filePath => _activeProcessor.SupportedExtensions.Contains(Path.GetExtension(filePath)))
		...
		private void btnStart_Click(object sender, EventArgs e) {
			_activeExperiment.ManuallyLearnUntilBeStable(
		...
		private bool AddNewElements() {
			var acceptedFeatureAndElements = _activeExperiment.SuspiciousAcceptedInAccepting
					.Select(t => Tuple.Create(t.Feature, _activeExperiment.Feature2Element[t.Feature]));
