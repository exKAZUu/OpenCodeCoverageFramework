...
public class GenerateCommand extends Command {
	 ...
	 */
	private static final String ABSTRACT_PAGE_PACKAGE = "com.google.testing.pogen.generator.test.java.page";
	...
	/**
	 * Constructs an instance with the the specified template paths, specified
	 * output-directory path, the specified package name, the specified
	 * attribute name and the boolean of the verbose mode.
	 * 
	 * @param templatePaths
	 *            the template paths to be parsed
	 * @param testOutDirPath
	 *            the output directory path of test codes
	 * @param packageName
	 *            the package name to generate skeleton test codes
	 * @param attributeName
	 *            the name of the attribute to be assigned for tags containing
	 *            template variables
	 * @param verbose
	 *            the boolean whether prints processed files verbosely
	 * @param rootDirectoryPath
	 *            the root directory of html template files
	 * @param templateFilePattern
	 *            the pattern for finding template files in the root directory
	 * @param isRecusive
	 *            the boolean whether template files are recursively found
	 */
	public GenerateCommand(String[] templatePaths, String testOutDirPath,
			String packageName, String attributeName, boolean verbose,
			String rootDirectoryPath, String templateFilePattern,
			boolean isRecusive) {
	...
	public void execute() throws IOException {
		...
		if (!rootInputDir.exists()) {
			throw new FileProcessException("Not found root intpu directory",
					rootInputDir);
		}

		// Generate the AbstractPage class
		File newAbstractPageFile = new File(testOutDir.getPath(),
				ABSTRACT_PAGE_NAME);
		if (!newAbstractPageFile.exists()) {
			URL abstractPageUrl = Resources.getResource(ABSTRACT_PAGE_NAME);
			String abstractPage = Resources.toString(abstractPageUrl,
					Charset.defaultCharset());
			abstractPage = abstractPage.replaceAll(ABSTRACT_PAGE_PACKAGE,
					packageName);
			Files.write(abstractPage, newAbstractPageFile,
					Charset.defaultCharset());
		} else if (verbose) {
			System.err.println("Already exists: "
					+ newAbstractPageFile.getAbsolutePath() + ".");
		}
		...
		if (!Strings.isNullOrEmpty(templateFilePattern)) {
			templateFiles.addAll(FileUtils.listFiles(rootInputDir,
					new RegexFileFilter(templateFilePattern),
					isRecusive ? FileFilterUtils.trueFileFilter() : null));
		}

		TemplateUpdater updater = TemplateUpdaters
				.getPreferredUpdater(attributeName);
		TestCodeGenerator generator = new TestCodeGenerator(attributeName);
		for (File file : templateFiles) {
			checkExistenceAndPermission(file, true, true);
			try {
				TemplateParser parser = TemplateParsers.getPreferredParser(
						file.getPath(), attributeName);
				...
				parseAndGenerate(file, rootInputDir, testOutDir, parser,
						updater, generator);
			} catch (TemplateParseException e) {
				throw new FileProcessException(
						"Errors occur in parsing the specified files", file, e);
			} catch (PageObjectUpdateException e) {
				throw new FileProcessException(
						"Errors occur in updating the specified files", file, e);
			}
		}
	}
	 ...
	 * 
	 * @param templateFile
	 *            the template file to be modified
	 * @param rootInputDir
	 *            the root input directory of template files
	 * @param codeOutDir
	 *            the output directory of skeleton test code
	 * @param parser
	 *            the parser to parse template files
	 * @param updater
	 *            the updater to update template files
	 * @param generator
	 *            the generator to generate skeleton test code
	 * @throws IOException
	 *             if errors occur in reading and writing files
	 * @throws TemplateParseException
	 *             if the specified template is in bad format
	 * @throws PageObjectUpdateException
	 *             if the existing test code doesn't have generated code
	 */
	private void parseAndGenerate(File templateFile, File rootInputDir,
			File codeOutDir, TemplateParser parser, TemplateUpdater updater,
			TestCodeGenerator generator) throws IOException,
		...
		String pageName = NameConverter
				.getJavaClassName(getFileNameWithoutExtension(templateFile));
		// Read template file
		String template = Files.toString(orgTemplateFile,
				Charset.defaultCharset());
		...
		URI relativeDirUri = rootInputDir.toURI().relativize(
				templateFile.getParentFile().toURI());
		String relativeDirPath = relativeDirUri.toString();
		if (relativeDirPath.endsWith("/")) {
			relativeDirPath = relativeDirPath.substring(0,
					relativeDirPath.length() - 1);
		}
		...
		if (codeFile.exists() && !codeFile.canWrite()) {
			throw new FileProcessException(
					"No permission for writing the specified file", codeFile);
		}

		// @formatter:off
		String testCode = codeFile.exists() ? generator.update(templateInfo,
				Files.toString(codeFile, Charset.defaultCharset())) : generator
				.generate(templateInfo, packageName + packagePrefix, pageName);
		// @formatter:on
		if (verbose) {
			System.out.print(".");
		}
	 ...
	 * @throws IOException
	 *             if errors occur in backuping files
	 */
